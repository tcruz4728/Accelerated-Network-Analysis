function outNoise = statgaussnoisegen(nSamples,psdVals,fltrOrdr,sampFreq, varargin) %noise_num, noisefile)
...
if nargin <4
    if ~isempty(varargin{1}) && ~isempty(varargin{2})
        noise = load(varargin{2});
        inNoise_t = noise.wgn(varargin{1},1:end);
    end
else
